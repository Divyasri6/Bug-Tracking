...
- Context-aware suggestions based on similar past bugs
- Automatic storage of resolved bugs for future learning
...
1. Frontend Request
   POST /ai/suggest
   {
     ...
     "description": "...",
     "resolution": null  // Optional: resolution notes if bug is resolved
   }
   ↓
2. FastAPI Receives Request
   - Validates BugQuery (title, description, optional resolution)
   - Extracts bug information
   ↓
3. Build Query Text for Retrieval
   query_text = title + " " + description
   (Resolution not included in query to find similar problems, not solutions)
   ↓
4. RAG: Retrieve Similar Bugs
   - Initialize vector store (lazy initialization with OpenAI API key)
   - Generate embedding for query_text (OpenAI Embeddings API)
   - Search ChromaDB vector store (similarity_search with k=3)
   - Get top 3 similar bugs from knowledge base
   ↓
5. Build Context Section
   - Format similar bugs into readable text
   - Include bug IDs and snippets (truncated to 350 chars)
   - If no similar bugs found, return "No relevant historical bug reports..."
   ↓
6. Build Prompts
   - System prompt (business analyst role - provides business impact focus)
   - Human prompt (bug details + context from similar bugs + instructions)
   - Format instructions (PydanticOutputParser for structured output)
   ↓
7. Call LLM (OpenAI)
   - Send system + human messages to ChatOpenAI
   - LLM generates response with context from similar bugs
   - Response includes suggestion and predictedPriority
   ↓
8. Parse Response
   - PydanticOutputParser validates and parses JSON
   - Returns AiSuggestion object with suggestion and predictedPriority
   ↓
9. Store Bug in Vector Store
   - Generate bug hash (SHA1 of title + description + resolution)
   - Create Document with title + description + resolution (if provided)
   - Add to ChromaDB vector store and persist to disk
   - Bug is now available for future RAG queries
   ↓
10. Return Response
    {
