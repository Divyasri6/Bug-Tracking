...
└── src/
    ...
    └── test/java/com/example/
        └── Bug/Tracking/      # Spring Boot test harness
```
...
The backend Dockerfile bundles the Spring Boot service **and** the AI FastAPI service in one container. Ensure the OpenAI API key is available before running.

```bash
# Build the image from the repository root
docker build -f backend/Dockerfile -t bug-tracker-backend .

# Run the container, exposing backend and AI ports
docker run \
  -p 8080:8080 \
  -p 5001:5001 \
...
- `AI_WORKERS` – number of Uvicorn workers for the AI service (default `1`).
...
| --- | --- | --- | --- |
| `POST` | `/bugs` | Create a new bug | JSON body with title, description, status, priority, assignedTo |
| `GET` | `/bugs` | List bugs | Optional query params: `status`, `priority` |
| `GET` | `/bugs/{id}` | Retrieve bug by ID | Returns 404 if not found |
| `PUT` | `/bugs/{id}` | Update bug | Partial updates supported; missing fields remain unchanged |
| `DELETE` | `/bugs/{id}` | Delete bug | Returns `204 No Content` |
...
- **Bug** – Core entity stored in the `bugs` table with audit timestamps and status/priority enums.
